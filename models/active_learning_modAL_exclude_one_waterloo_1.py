...
def train_for_user(fd, user_id=1, n_class=5, data_id=None):
    test_data = waterloo_i_processing.get_all_but_one_user(ex_user_id=data_id)
...
def sys_main():
    ...
    fd = open('results/misc_data_folder/modAL-result-exclude-one-model-1-10.txt', 'w')
    E1 = []
    E2 = []
    E3 = []
    sd1 = []
    ...
    np.savetxt('results/tt_modAL-result-exclude-one-user-numpy-waterloo_1_err_e1.txt', E1, delimiter=',')
    np.savetxt('results/tt_modAL-result-exclude-one-user-numpy-waterloo_1_sd_e1.txt', sd1, delimiter=',')
