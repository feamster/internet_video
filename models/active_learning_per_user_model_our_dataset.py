...
def split_train_test(X, y, test_size, n_class):
    ...
    init_y_ind = np.array([np.where(y_trn == i)[0][0] for i in range(0, len(target))])
...
def train_for_user(user_id=None, n_class=None):
    X, y = sensei_data.extract_data(usr_id=user_id)
    y = y - 1
    n_class = np.max(y) + 1
    print(n_class)
    ...
    save_file('results/'+'ours_user_'+str(user_id)+'_E1_class_'+str(n_class)+'.txt', result['E1'])
    save_file('results/'+'ours_user_'+str(user_id)+'_E2_class_'+str(n_class)+'.txt', result['E2'])
    save_file('results/'+'ours_user_'+str(user_id)+'_E3_class_'+str(n_class)+'.txt', result['E3'])
...
def sys_main():
    ...
    for usr in range(9, len(usr_list)):
        print('User:', usr_list[usr])
        train_for_user(user_id=usr_list[usr], n_class=n_of_class)
    return 0
